...
sun = Ball([250,250], [0,0], [0,0], 5, 50)
#planet1 = Ball([250, 100], [1, 0], [0, 0], 1, 10)

def gravity(planet):
    G = 0.002
    force = -(G * planet.m * sun.m) / rel_dist(planet, sun)
    return force
...
def update_acc():
    ...
    for planet in ball_set:
        if planet != sun:
            ...
            planet.acc = (force / planet.m) * rel_vec(planet, sun)
...
while not done:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            ball_set.append(Ball(pos, [1, 0], [0, 0], 1, 10))
