...
def collisionDetectBalls():
    ...
    for (ball1, ball2) in collision_list:
        ...
        v1n = v1 - (2*ball2.m)/(ball1.m+ball2.m) * (np.dot(v1 - v2,
                                                           p1 - p2)/(np.linalg.norm(p1 - p2)**2))*(p1 - p2)
        v2n = v2 - (2*ball1.m)/(ball1.m+ball2.m) * (np.dot(v2 - v1,
                                                           p2 - p1)/(np.linalg.norm(p2 - p1)**2))*(p2 - p1)
...
def attraction(ball1, ball2):
    ...
    force = (G * ball1.m * ball2.m / np.linalg.norm(p1-p2)) * ((p1 - p2)/np.linalg.norm(p1 - p2))

    ball1.vel = v1 - (force / ball1.m)
    ball2.vel = v2 + (force / ball2.m)
...
for i in range(2):
    for j in range(2):
        ...
        ball = Ball((x, y), (0, 0), (0, 0), 1, ball_size)
...
while not done:
    ...
    attraction(ball_set[0], ball_set[1])
